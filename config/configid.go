...
import (
	...
	psmp "github.com/hyperledger/fabric-sdk-go/pkg/common/providers/msp"
...
func InitializeUserIdentity() {
	...
	mspClient, err := msp.New(Sdk.Context(), msp.WithOrg(Vars.OrgId))
	if err != nil {
		panic(errors.Errorf("Error creating MSP client: %s", err))
	}

	if PrivateKey == "" {
		User, err = mspClient.GetSigningIdentity(Vars.UserName)
		if err != nil {
		...
		if Cert == nil {
			if Cert, err = GetSigncertsBytes(Vars.OrgId); err != nil {
				panic(errors.Errorf("Nope: %v", err))
			}
		}

		// pvtKey must be decrypted when it is passed in function CreateSigningIdentity.
		User, err = mspClient.CreateSigningIdentity(psmp.WithCert(Cert), psmp.WithPrivateKey([]byte(PrivateKey)))
		if err != nil {
...
func SetClientTlsKey(tlsKey string) {
	vc := viper.New()
	vc.SetConfigFile(Vars.ChannelConfigPath)
	err := vc.ReadInConfig()
	if err != nil {
		panic(fmt.Sprintf("Failed to read configFile: %s", Vars.ChannelConfigPath))
	}
